...
Begin VB.Form Form1 
   ...
   Begin VB.CommandButton Command15 
      ...
      Width           =   615
   End
   Begin VB.CommandButton QJButton 
      Caption         =   "下位"
      Height          =   615
      Index           =   1
      Left            =   14160
   ...
   Begin VB.CommandButton QJButton 
      ...
      Width           =   495
   End
   Begin VB.CommandButton qjwhbutton 
      Caption         =   "尾号区间"
      Height          =   615
      Left            =   13800
      TabIndex        =   145
...
Dim RegLostFlag As Boolean

Dim WeiQu As Integer
Dim SoftWareModel  As String
...
Dim Sum4(6) As Integer
Dim Sum5(6) As Integer
Dim Sum6(6) As Integer
Dim SumM(MM) As String
Dim j4, j5, j6 As Integer
Dim mm1 As Integer
List2.Clear
List4.Clear
List2.Width = List2_Width \ 4 + 350
...
For i = List1.ListCount - MM To List1.ListCount - 1
    tempstr = SubStr(Mid(List1.List(i), 6, 14), Mid(List1.List(i - 1), 6, 14), SubSum)
  '  tempstr = Format(tempstr, "@@@")
    
    For j = 1 To 6
    ' Sum4(j) = Sum4(j) + Val(Mid(tempstr, 4 * j - 3, 4))
     Sum5(j) = Sum5(j) + Val(Mid(tempstr, 4 * j - 3, 4))
     Sum6(j) = Sum6(j) + Val(Mid(tempstr, 4 * j - 3, 4))
    Next j
    j4 = j4 + 1
    j5 = j5 + 1
    j6 = j6 + 1
     
    ' List4.AddItem GetSumStr(Sum4())
    If j4 = 4 Then
        j4 = 0
     '   List4.AddItem GetSumStr(Sum4())
      Call ClearSumStr(Sum4())
          
    End If
    If j5 = 5 Then
    '    List4.AddItem GetSumStr(Sum5())
    End If
    If j6 = 7 Then
     '   List4.AddItem GetSumStr(Sum6())
    End If
    
    
    List2.AddItem tempstr + "|" + Str(SubSum)
    SubSum = 0
Next i

For i = List1.ListCount - 1 To List1.ListCount - MM Step -1
 tempstr = SubStr(Mid(List1.List(i), 6, 14), Mid(List1.List(i - 1), 6, 14), SubSum)
 For j = 1 To 6
  Sum4(j) = Sum4(j) + Val(Mid(tempstr, 4 * j - 3, 4))
 Next j
  mm1 = mm1 + 1
  SumM(mm1) = GetSumStr(Sum4())
  
 Next i
 For i = mm1 To 1 Step -1
  List4.AddItem SumM(i)
Next i
'List4.AddItem tempstr

End Sub
Private Function GetSumStr(Sum() As Integer) As String
Dim i As Integer
For i = 1 To 6
 GetSumStr = GetSumStr + Format(Str(Sum(i)), "@@@") + " "
Next i
End Function
Private Function ClearSumStr(Sum() As Integer)
Dim i As Integer
For i = 1 To 6
 Sum(i) = 0
 Next i
...
SoftWareModel = "Tail Model "
 For i = List1.ListCount - 100 To List1.ListCount - 1
    ...
    TailRegStr = Str(Tail(1) + Tail(2) + Tail(3)) + Str(Tail(4) + Tail(5) + Tail(6)) + Str(Tail(7) + Tail(8) + Tail(9) + Tail(0))
    ...
    Next k
         ...
         If Not (Tail(0) = 0) Then
      TailCount(0) = TailCount(0) + Tail(0)
      LineStr = LineStr + Str(0) + Str(Tail(0)) + " | "
      TailAll = TailAll + 1
      TailStr = TailStr + Format(Str(0), "0")
      If Not (TailPre(0) = 0) Then
        PreCount = PreCount + 1
      End If
     Else
      LineStr = LineStr + "    " + " | "
    ...
    List2.AddItem LineStr + TailRegStr + "|" + Str(TailAll) + "|" + Str(PreCount) + "|" + TailStr
    TailStr = ""
    LineStr = ""
    TailAll = 0
    PreCount = 0
...
Private Sub GuessTailSum()
Dim tempstr As String
Dim N(9) As Integer
'Dim TailRegStr(3)(16) as String
Dim i, j, k, l As Integer
Dim a1, a2, a3 As Integer
tempstr = InputBox("区间和值", "区间和值")
a1 = Val(Mid(tempstr, 1, 1))
a2 = Val(Mid(tempstr, 2, 1))
a3 = Val(Mid(tempstr, 3, 1))
For i = 0 To 2
    For j = 0 To 2
        For k = 0 To 2
            If i + j + k = a1 Then
...
'PopupMenu Color, vbPopupMenuLeftButton
...
Private Function Check123(IStr As String) As String
Dim i As Integer
Dim ival As Integer
Dim C123(5) As Integer
For i = 0 To 5
   ival = Val(Mid(IStr, 2 * i + 1, 2))
   If ival >= 0 And ival < 4 Then
    C123(ival) = C123(ival) + 1
   End If
   If ival >= 4 And ival < 10 Then
    C123(4) = C123(4) + 1
   End If
   If ival >= 10 Then
    C123(5) = C123(5) + 1
...
Next i
Check123 = Str(C123(0)) + Str(C123(1)) + Str(C123(2)) + Str(C123(3)) + Str(C123(4)) + Str(C123(5))
End Function
Private Function Prob123() As String
Dim i, j As Integer
Dim C123(5) As Integer
Dim ival As Integer
Dim tempstr As String
Dim Sum123 As Integer
For i = 0 To List1.ListCount - 1
tempstr = Replace(Mid(List1.List(i), 27, 18), " ", "")
For j = 0 To 5
   ival = Val(Mid(tempstr, 2 * j + 1, 2))
   If ival >= 0 And ival < 4 Then
    C123(ival) = C123(ival) + 1
   End If
   If ival >= 4 And ival < 10 Then
...
'Prob123 = Str(C123(0)) + Str(C123(1)) + Str(C123(2)) + Str(C123(3)) + Str(C123(4)) + Str(C123(5))
Sum123 = C123(0) + C123(1) + C123(2) + C123(3) + C123(4) + C123(5)
Prob123 = Str(C123(0) / Sum123) + Str(C123(1) / Sum123) + Str(C123(2) / Sum123) + Str(C123(3) / Sum123) + Str(C123(4) / Sum123) + Str(C123(5) / Sum123)
...
'' 遗漏区间分布 temp1230  0 区间       1  区间  2  区间  3 区间   4-9 区间  5 >10 区间
Private Sub Command1799999_Click()
Dim i, j As Integer
Dim tempstr As String
...
SavePicture tmpPicture, App.Path + "\" + SoftWareModel + Format(Now, "yyyymmddhhmmss") & ".BMP"
...
Dim i, j As Integer
Dim iPre As Integer
Dim REGS(1000) As String
Dim RegList(1000) As String
List2.Clear
Dim Rtotal As Integer
j = 0
For i = List1.ListCount - 1 To 0 Step -1
    REGS(j) = Mid(List1.List(i), 66, 7)
    j = j + 1
Next i
 Rtotal = j
For i = 0 To Rtotal
...
For i = 0 To 2
 ...
 Call CheckAttrib(IStr, NumAttrib)          ' 得到 当前数字的大小 奇数 偶数特性
   ...
   ' if numattrib.BSAttrib
       AppNums = CheckAppReg(IStr, AppregStr)      ' AppearStr 出现分区分布 ， AppNums  区间数值
       AppNumFlag = False
       For j = 0 To 2
       If Option2(j).Value Then
         If AppNums = j + 3 Then
            AppNumFlag = True
            Exit For
         End If
   ...
   '**********************************************************
    ...
    If OddFlag And BigFlag _
      ...
      ' List2.AddItem Istr + " " + NumAttrib.BSAttrib + " " + NumAttrib.OEAttrib + " " + NowlostStr + " " + Str(NowLostSum) + " " + Format(Str(NowSum), "000") + " " + AppregStr
        CheckAllData = NumAttrib.BSAttrib + " " + NumAttrib.OEAttrib + " " + NowlostStr + " " + Str(NowLostSum) + " " + Format(Str(NowSum), "000") + " " + AppregStr
    Else
        CheckAllData = ""
     End If
...
If AddFrame16 Then
    AddFrame16 = False
    Form1.Height = Form1.Height - Frame16.Height
Else
    AddFrame16 = True
    Form1.Height = Form1.Height + Frame16.Height
End If
...
End Sub
Private Sub ShowLabel6()
Dim i, j, k As Integer
Dim Sum5(4) As Integer
Dim tempstr As String
Dim LineStr As String
For i = List1.ListIndex - 49 To List1.ListIndex
 tempstr = Right(List1.List(i), 4)
 For j = 1 To 4
  Sum5(j) = Sum5(j) + Val(Mid(tempstr, j, 1))
 Next j
 k = k + 1
 If k = 5 Then
 k = 0
  For j = 1 To 4
   LineStr = LineStr + Str(Sum5(j))
   Sum5(j) = 0
  Next j
    List2.AddItem LineStr
    LineStr = ""
...
'Dim L4index(50) As Integer
Dim TempSum As Integer
Dim i As Integer
Dim Spos As Integer
On Error Resume Next        '错误自动返回


If Check8.Value = 1 Then
    Spos = InStr(List2.List(List2.ListIndex), " ")
    TempSum = AddStrSum(Mid(List2.List(List2.ListIndex), 1, Spos))
  For i = 0 To List2.ListCount - 1
    List4.Selected(i) = False
   If TempSum = AddStrSum(Mid(List2.List(i), 1, Spos)) Then
...
If KeyCode = vbKeyH Then
 MsgBox "区间尾号分布指标                            " + vbCrLf + _
  "重合码数字为   1 3  6  8 " + vbCrLf + _
  "区间位置99%概率数值" + vbCrLf + _
  " 1区间 1-15 " + vbCrLf + _
 " 2区间 2-20 " + vbCrLf + _
 " 3区间 4-26" + vbCrLf + _
 " 4区间 7-30 " + vbCrLf + _
...
Private Sub QJButton_Click(Index As Integer)
...
Else
 ...
 ElseIf WeiQu > 5 Then
  WeiQu = 0
 End If
 Call qjwhbutton_Click
 
End Sub

Private Sub qjwhbutton_Click()
Dim i, j As Integer
Dim tempstr As String
Dim LineStr As String
Dim TempZB As ZhiBiao
Dim TempData(7) As Integer
List2.Clear
'                                         012     | 3456|     789|0 3 6 9|1 4 7| 2 5 8 |前面相交| |
'红号 区间排号统计

'99% 区间作用   01-15     02 -20    04 -26  07 -30   12-32     20-33
List2.AddItem "尾号区间统计     |小数| 中数 |大数 ||| 0路 | 1路  |2路 |||  重合码  |||   大小数  |||   偶奇数  |||   质合数  |"

For i = List1.ListCount - 100 To List1.ListCount - 1
 tempstr = Mid(List1.List(i), 6, 14)
 'List2.AddItem tempstr
 For j = 0 To 5
  TempData(j) = Val(Mid(tempstr, j * 2 + 1, 2))
 Next j
Call CheckZB(TempData(WeiQu), TempZB)
List2.AddItem tempstr + " | " + ShowZBR(TempZB) + Mid(tempstr, 2 * WeiQu + 1, 2)

Next i
'99% 区间作用   01-15     02 -20    04 -26  07 -30   12-32     20-33
List2.AddItem "尾号区间  " + Str(WeiQu + 1) + "统计 " + " |小数| 中数 |大数 ||| 0路 | 1路  |2路 |||  重合码  |||   大小数  |||   偶奇数  |||   质合数  |"
List2.ListIndex = List2.ListCount - 1
